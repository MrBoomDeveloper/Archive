const projects = {
	"pinned": [{
		"banner": "images/banners/boomstudio.jpg",
		"name": "BoomStudio",
		"tags": ["Java", "Android", "В разработке", "2022"],
		"screenshots": ["images/not_found.jpg", "images/not_found.jpg", "images/not_found.jpg"],
		"description": "Мобильная среда разработки, используемая для редактирования файлов, компиляции проектов и т.д. Данное приложение позволяет создавать проекты на различных языках программирования. А благодаря системе плагинов, можно добавлять в редактор новые функции. Каждый человек может создать свой плагин без использования дополнительных программ.",
		...
		case "archive":
			...
			for(var i = 0; i < json.length; i++) {
				...
				for(var i1 = 0; i1 < json[i].content.length; i1++) {
					...
					html += tags_html + '</p><p class="link">' + json[i].content[i1].link + '</p></div></div></a>';
				}
				html += '</div></div></div>';
			}
