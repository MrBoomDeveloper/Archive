...
const boom = {
	...
	loadDesign: () => {
		if (!boom.online)
			console.error("Please, connect to the internet!")
	...
	log: text => {
		boom.alert("Console Log", text, 2000)
	},
	error: text => {
		boom.alert("An error has occurred", text, 5000)
	},
	warn: text => {
		boom.alert("Sorry", text, 3000)
	},
	debug: (text, id) => {
		new BoomDialog(`dialog-debug-` + id).text(`<textarea neon>${text}</textarea>`)
	}
}
...
window.addEventListener("error", () => {
	boom.error("Please check the console")
})
